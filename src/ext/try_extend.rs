...
pub trait TryExtend<A> {
    ...
    /// Tries to extend the collection with the values from the provided iterator.
    ///
    /// # Errors
    ///
    /// If the collection runs out of capacity or for example one tries to insert
    /// elements that should not be inserted.
